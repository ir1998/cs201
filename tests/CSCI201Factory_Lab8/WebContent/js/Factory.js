...
function Factory(factoryData) {
	...
	this.drawMailbox(factoryData.mailbox);
	this.drawWorkbench(factoryData.workbench)
	this.onresize();
}

Factory.prototype.moveWorker = function (worker, shortestPathStack) {
	var workerImg = document.getElementById(worker.name);
	workerImg.src = 'img/' + worker.image;
...
Factory.prototype.drawMailbox = function (mailbox) {
	var x = mailbox['x'],
	y = mailbox['y'];
	var cell = this.simulation.rows[y].cells[x];

	new PlaceObject(cell, mailbox);
}

Factory.prototype.drawWorkbench = function (workbench) {
	var x = workbench['x'],
	y = workbench['y'];
	var cell = this.simulation.rows[y].cells[x];

	new PlaceObject(cell, workbench);
}
