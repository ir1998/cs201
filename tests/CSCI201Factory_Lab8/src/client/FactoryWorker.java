...
public class FactoryWorker extends FactoryObject implements Runnable {
	...
	// references
	protected transient Factory mFactory;
	protected transient FactoryProduct mProductToMake;

	// threading
	protected transient Lock mLock;
	protected transient Condition atLocation;
	protected transient Thread thread;

	// path finding
	protected FactoryNode currentNode;
	protected transient FactoryNode mDestinationNode;
	protected transient Stack<FactoryNode> mShortestPath;
	...
	public void run() {
		...
		try {
			while (true) {
				...
				// get the resources to build the product
				...
				// go to the workbench to build the product
				{
					FactoryNode workbenchNode = mFactory.getNode(Constants.workbenchString);
					mShortestPath = currentNode.findShortestPath(workbenchNode);
