	...
	<div id = "small_feed">
		...
		<table>
			<tbody>
				...
				<td> <img src = <%= StringConstants.ACTION_IMG_EXT+event.getActionIcon() %>> <%=event.getAction().toLowerCase() + " "%> <a href = "${pageContext.request.contextPath}/jsp/movie_profile.jsp?<%=StringConstants.TITLE%>=<%= title %>" class = "movie_title"> <%= event.getMovieTitle() %> <a style="font-size:14px"><%=event.getTime()%></a></td>
				<% } %>
				</tr>
			</tbody>
		</table>
